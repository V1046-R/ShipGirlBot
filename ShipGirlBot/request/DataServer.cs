...
{
    private string choosedServerAddress = "http://hmtest.jianniang.com/";
    public string cookie = null;
    private long lastCallTime;
    private string loginServer;
    public string mainServerAddress = "http://hmtest.jianniang.com/";
    ...
    public WWW GetWWWOf(string path, Dictionary<string, string> ddd = null, ServerType useServerType = 0, bool useZip = false, string extraInfo = "")
    {
        ...
        if (ddd != null)
        {
            string str = new JsonWriter().Write(ddd);
            www = new WWW(choosedServerAddress + path + str2 + str3 + str4 + extraInfo, this.GetBytes(str), headers, cookie,useZip);
        }
        else
        {
            www = new WWW(choosedServerAddress + path + str2 + str3 + str4 + extraInfo, null, headers, cookie, useZip);
        }
